...
def get_animal_history(*, session: Session = Depends(get_session), animal_id: int) -> dict:
    ...
    history = get_animal_history(session, animal_id)
    if not history:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Animal not found")
    return history
...
def get_animal_by_rfid(*, session: Session = Depends(get_session), rfid_code: str) -> dict:
    """Lookup an animal by RFID and return its history and last known lot."""
    result = get_animal_by_rfid(session, rfid_code)
    if not result:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Animal not found")
    return result
